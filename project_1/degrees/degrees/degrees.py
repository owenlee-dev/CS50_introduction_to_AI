...
def shortest_path(source, target):
    ...
    # keep looping as long as the frontier is not empty
    while flag:
      if frontier.empty():
          return None
      ...
      for movie, actor in neighbors:
          # if neighbor not in the frontier and hasn't already been explored
          if actor not in explored and not frontier.contains_state(actor):

            # create a node for neighbor
            neighbor_node=Node(state=actor, parent=current, action=movie)
            
            # if neighbor is the target
            if neighbor_node.state==target :
              # set current to the neighbor and begin recursing
          ...
          # if not in frontier, hasn't been explored and isn't the target add it 
